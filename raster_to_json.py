...
def raster_to_json(line, print_door_warning):
    ...
    if not Path("rplan_json").exists():
        Path("rplan_json").mkdir()
...
def main():
    ...
    if Path(line).is_dir():
        logging.info(f"Processing directory: {line}")
        for file in Path(line).glob("*.png"):
            fp_id = str(file).split("/")[-1].split(".")[0]
            if (
                Path(f"rplan_json/{fp_id}.json").exists()
                or Path(f"failed_rplan_json/{fp_id}").exists()
            ):
                continue
            try:
                raster_to_json(str(file), print_door_warning=False)
            except (AssertionError, ValueError, IndexError) as e:
                if not Path("failed_rplan_json").exists():
                    Path("failed_rplan_json").mkdir()
